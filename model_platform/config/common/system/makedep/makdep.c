...
** default "make" procedure for C-Coupler platform. This code was originally 
** from CCM or CCSM, and then upgraded by Dr. Li Liu (liuli-cess@tsinghua.edu.cn)   
**
...
struct mod_src_map_node {
  char src_file[MAXLEN];
  char module[MAXLEN];
  struct mod_src_map_node *next;
};
/* 
...
main (int argc, char **argv)
{
  ...
  char depnam_special[MAXLEN];   /* dependency name (from #include or use) */
  ...
  char mod_file_head[MAXLEN];
  struct mod_src_map_node *mod_src_map_head, *mod_src_map_curr;
  /*
  ...
  */
  ...
  if ((fp_mod_src_map = fopen ("module_mapping.info", "r")) == NULL) {
    fprintf (stderr, "Can't open file module_mapping.info\n");
    exit (1);
  }

  length = 0;
  counter = 0;
  mod_src_map_curr = malloc (sizeof(struct mod_src_map_node));
  mod_src_map_head = NULL;
  current_str = mod_src_map_curr->src_file;
  while (!feof(fp_mod_src_map) && (cc = getc(fp_mod_src_map)) != -1) {
    current_str[length++] = cc;
    if (cc == ' ') {
      current_str[length-1] = '\0';
      counter ++;
      length = 0;
      current_str = mod_src_map_curr->module;
    }
    if (cc == '\n' || feof(fp_mod_src_map)) {
      if (counter != 1) {
        fprintf (stderr, "Error in analysis of mapping between source files and modules\n");
        exit (1);
      }
      current_str[length-1] = '\0';
      mod_src_map_curr->next = mod_src_map_head;
      mod_src_map_head = mod_src_map_curr;
      counter = 0;
      length = 0;
      mod_src_map_curr = malloc (sizeof(struct mod_src_map_node));
      current_str = mod_src_map_curr->src_file;
	...
	if ( ! already_found (depnam, uselist)) {  /* Skip any duplicates */
	  ...
	  */
	  ...
	  for (sptr = suffix_list; sptr != NULL; sptr = sptr->next) {
        ...
        /* Search the source file name according to module name */
        module_file_name = NULL;
        mod_src_map_curr = mod_src_map_head;
        while (mod_src_map_curr != NULL) {
          if (strcasecmp(mod_src_map_curr->module, depnam) == 0) {
            module_file_name = mod_src_map_curr->src_file; 
            break;
          } 
          mod_src_map_curr = mod_src_map_curr->next;
        }
        if (!(module_file_name != NULL && strcmp(sptr->name, module_file_name+strlen(module_file_name)-strlen(sptr->name)) == 0)) 
          continue;

        strcpy (mod_file_head, module_file_name);
        mod_file_head[strlen(module_file_name)-strlen(sptr->name)] = '\0';
	    strcpy (srcfile, mod_file_head);
