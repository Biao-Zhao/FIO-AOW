...
sed -i "s#entry id *= *\"RUNDIR\" *value *=.*#entry id=\"RUNDIR\"   value = \"$COMP_RUN_DATA_DIR/\" \/\>#" env_run.xml
...
./cesm_setup -clean >> cesm_configure.log
echo ""   > cesm_configure.log
if [ -e LockedFiles/env_mach_pes.xml.locked ]; then
if ! diff env_mach_pes.xml LockedFiles/env_mach_pes.xml.locked > /dev/null ; then
    ./$CASE_NAME.clean_build  >> cesm_configure.log
fi
fi

./cesm_setup >> cesm_configure.log


if [ $rewrite_config = 1 ]; then
    sed -i "s#orb_iyear .*=.*# orb_iyear = $ORBYEAR#" CaseDocs/drv_in
    sed -i "s#orb_iyear_align.*=.*# orb_iyear_align = $ORBYEAR#" CaseDocs/drv_in
    sed -i "s#orb_iyear *=.*# orb_iyear = $ORBYEAR#" Buildconf/cplconf/drv_in
    sed -i "s#orb_iyear_align.*=.*# orb_iyear_align = $ORBYEAR#" Buildconf/cplconf/drv_in
    ...
    sed -i "s#setenv LID.*#setenv LID \"$configuration_time\"#" $CASE_NAME.build 
fi
