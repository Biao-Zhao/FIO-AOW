...
!GET INTEGER TYPE DATA
 ...
 IF(Status==-43)THEN
! WRITE(*,*)TRIM(Var_NAME),TRIM(ATT_NAME),' not exist'
 FIG=1
 ELSE
 ...
 IF(Status==-43)THEN
! WRITE(*,*)TRIM(Var_NAME),TRIM(Fact_NAME),' not exist'
 FACT=1.
 ELSE
...
! WRITE(*,*)TRIM(VAR_NAME),TRIM(ATT_NAME),' not exist'
 ...
 Status=NF_INQ_ATTID(NcID,varID,Trim(Fact_NAME),AttID)
 IF(Status==-43)THEN
! WRITE(*,*)TRIM(FACT_NAME),TRIM(Fact_NAME),' not exist'
 FACT=1.
 ELSE
 Status=NF_GET_ATT_REAL(ncID,varID,trim(Fact_NAME),FACT)
 IF(Status.NE.NF_NOERR) CALL HANDLE_ERR(Status)
...
!----------------------------------------------------------------------
 ...
 IF(Status==-49)THEN
! difine 2d data
! PRINT*,"DEFINE NEW VARIABLES ",TRIM(VN)
