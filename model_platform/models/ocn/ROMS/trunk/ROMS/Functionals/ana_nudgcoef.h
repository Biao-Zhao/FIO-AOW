...
!
      ...
      real(r8), dimension(IminS:ImaxS,JminS:JmaxS) :: wrk_cc_ts !ydz 2017.12.22
...
!
      DO j=JstrT,JendT
        DO i=IstrT,IendT
          wrk(i,j)=0.0_r8
          wrk_cc_ts(i,j)=1.0_r8   !ydz 2017.12.22
        END DO
      END DO
...
#elif defined nudcoef_YDZ_PACIFIC
!  Set tracer, surface height momentum et al nudging coefficients 
!  in the southern , northern edges,western and eastern edges 
!  from a user(4) days time scale at the boundary point to decrease
!  linearly to user(5) days Iwrk grids points away from the boundary.

!  ----------ydz:cc function change from 1.0 ---> 0.  ------
!    print*,'------Yangdezhou-------------',INT(user(1)),Iwrk,REAL(user(3),r8)
!
      Iwrk=INT(user(2))
      cff1=REAL(user(4),r8)  ! boundary YDZ
      cff2=REAL(user(5),r8)  ! interior YDZ
!--------- South boundary 
!
      DO j=JstrT,MIN(Iwrk,JendT)  !South boundary 
        DO i=IstrT,IendT
          wrk(i,j)=cff2+REAL(Iwrk-j,r8)*(cff1-cff2)/REAL(Iwrk,r8)
          wrk_cc_ts(i,j)=wrk(i,j)   !ydz 2017.12.22
        END DO
      END DO
!--------- North boundary 
!
      DO j=MAX(JstrT,Mm(ng)+1-Iwrk),JendT  !North boundary
        DO i=IstrT,IendT
          wrk(i,j)=cff1+REAL(Mm(ng)+1-j,r8)*(cff2-cff1)/REAL(Iwrk,r8)
          wrk_cc_ts(i,j)=wrk(i,j)   !ydz 2017.12.22
        END DO
      END DO
!--------- West boundary       
!
      DO i=IstrT,MIN(Iwrk,IendT)    !West boundary
        DO j=MAX(JstrT,i),MIN(Mm(ng)+1-i,JendT)
          wrk(i,j)=cff2+REAL(Iwrk-i,r8)*(cff1-cff2)/REAL(Iwrk,r8)
          wrk_cc_ts(i,j)=wrk(i,j)   !ydz 2017.12.22
        END DO
      END DO
!--------- East boundary     
!  
      DO i=MAX(IstrT,Lm(ng)+1-Iwrk),IendT   !East boundary
        DO j=MAX(JstrT,Lm(ng)+1-i),MIN(Mm(ng)+1-(Lm(ng)+1-i),JendT)
          wrk(i,j)=cff1+REAL(Lm(ng)+1-i,r8)*(cff2-cff1)/REAL(Iwrk,r8)
          wrk_cc_ts(i,j)=wrk(i,j)   !ydz 2017.12.22
        END DO
      END DO

!
!----------------using wrk to construct nudging coefficient---------------   
!-------                              
      if (ANY(LnudgeTCLM(:,ng))) THEN
        DO itrc=1,NTCLM(ng) !------- itemp isalt,iner are the same 
          DO k=1,N(ng)
            DO j=JstrT,JendT
              DO i=IstrT,IendT
                 CLIMA(ng)%Tnudgcof(i,j,k,itrc)=wrk_cc_ts(i,j)*Tnudg(itrc,ng)
